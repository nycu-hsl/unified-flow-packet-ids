...
if [ $dep_status = "No resources found in default namespace." ]; then
	...
	echo "============================="
	kubectl apply -f ~/APM.app/microservices-demo/release/kubernetes-manifests.yaml
	...
	echo "============================="
	helm install socialnetwork ~/APM.app/DeathStarBench/socialNetwork/helm-chart/socialnetwork/ --set-string global.imagePullPolicy=Always --set global.mongodb.sharding.enabled=false,global.mongodb.standalone.enabled=true --timeout 10m0s
  ...
  echo ""
	...
	echo "============================="
	kubectl apply -f ~/APM-ITRI/dvwa_deployment/01-mariadb.yaml
	kubectl apply -f ~/APM-ITRI/dvwa_deployment/01-app.yaml
